...
std::vector<GameCharacter *> createMonsters(int startX, int startY, const int numMonsters, const Dungeon &map) {
    ...
    result.reserve(numMonsters); //riservo, è buona norma, così non si rischia che lo spazio di fila finisca, obbligando il computer a spostare blocchi di dati
...
GameCharacter &selectEnemy(GameCharacter &hero, std::vector<GameCharacter *> &enemies) {
    int maxDistance = 1;
    for (auto enemy: enemies)
        if ((abs(hero.getPosX() - enemy->getPosX()) <= maxDistance) || (
                abs(hero.getPosX() - enemy->getPosY()) <= maxDistance))
            return *enemy;
}
