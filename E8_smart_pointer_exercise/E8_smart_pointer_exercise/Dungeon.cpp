...
void Dungeon::saveToFile(std::string fileName) throw(GameFileException) {
    ...
    try {
        ...
        std::cerr << e.what();
        throw GameFileException("Can not save map file", fileName, false);
    }
}
...
void Dungeon::loadFromFile(std::string fileName) throw(GameFileException, std::runtime_error, std::out_of_range) {
    ...
    try {
        ...
        throw GameFileException("Can not open map file", fileName, true);
    }
    std::string fileLine;
    ...
    if (y!=ysize)
        throw std::runtime_error("Map height size wrong. Expected "+std::to_string(ysize)+" but read: " + std::to_string(y));
    in.close();
}
