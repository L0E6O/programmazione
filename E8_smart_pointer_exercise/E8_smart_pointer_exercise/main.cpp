...
int main() {
    ...
    switch (playerType) {
        case PlayerType::KNIGHT:
            hero = std::make_unique<Knight>("Isildur", 34);
            break;
        case PlayerType::WIZARD:
            hero = std::make_unique<Wizard>("Gandalf", 35);
            break;
        case PlayerType::MAGE_KNIGHT:
            hero = std::make_unique<MageKnight>("Boromir", 35, 20, 12, 5);
            break;
    }
    ...
    std::vector<GameCharacter*> enemies = createMonsters(hero->getPosX(), hero->getPosY(), NUM_MONSTERS, map);
